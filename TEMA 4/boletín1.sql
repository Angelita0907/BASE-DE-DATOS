...
constraint pk_dniEmp2 primary key (DNI_EMPLE),
constraint fk_univ1 foreign key (COD_UNIV) references UNIVERSIDAD(COD_UNIV),
constraint pk_uni2 primary key (COD_UNIV)
);
...
FECHA_FIN DATE,
DNI_SUPERIOR CHAR(9),
constraint fk_dni3 foreign key (DNI_EMPLE) references EMPLEADO(DNI),
constraint pk_dniEmp3 primary key (DNI_EMPLE),
constraint fk_cod1 foreign key (COD_TRABAJO) references TRABAJO(COD_TRABAJO),
constraint pk_cod2 primary key (COD_TRABAJO),
constraint fk_dpto1 foreign key (COD_DPTO) references DEPARTAMENTO(COD_DPTO),
constraint pk_dpto2 primary key(COD_DPTO)
);
